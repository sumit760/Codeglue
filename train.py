...
class_weights = torch.tensor(
    np.load(os.path.join(DATA_DIR, "class_weights.npy")),
    dtype=torch.float32,
)
print(
...
if not USE_QLORA:
    ...
    raw_q = _ld(DATASET_NAME).map(lambda x: {"target": int(x["target"])})
    raw_q = raw_q.map(clean_code, desc="Cleaning (QLoRA)")

    def tokenize_qlora(batch):
        return tokenizer_qlora(batch["func"], truncation=True,
                               max_length=MAX_LENGTH, padding=False)

    tokenized_qlora = raw_q.map(
        tokenize_qlora, batched=True,
        remove_columns=["func", "id", "project", "commit_id"],
        desc="Tokenizing QLoRA",
    )
    ...
    wandb.init(project=WANDB_PROJECT, name="run-qlora-v1", reinit=True, config=qlora_config)
